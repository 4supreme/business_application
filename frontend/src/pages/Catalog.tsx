    ...
    <div className="grid md:grid-cols-2 gap-4">
      <div className="card">
        ...
        <div className="mt-3 flex gap-2">
          <button className="btn-line" onClick={()=>{ setName(""); setSku(""); setUnit("шт"); setBarcode(""); }}>
            Очистить
          </button>
          <button className="btn" onClick={async ()=>{
            if (!name.trim()) return alert("Введите название");
            await api.createProduct({ name, sku: sku||null, unit: unit||"шт", barcode: barcode||null });
            setName(""); setSku(""); setUnit("шт"); setBarcode(""); load();
          }}>
            Создать
          </button>
        </div>
      ...
      <div className="card">
        ...
        <table className="table">
          ...
          <tbody>
            {list.map((p: Product)=>(
              <tr key={p.id}><td>{p.id}</td><td>{p.name}</td><td>{p.unit||""}</td><td>{p.barcode||""}</td></tr>
            ))}
          </tbody>
        </table>
      </div>
